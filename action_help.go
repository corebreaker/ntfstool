...
func do_help(arg *tActionArg) error {
	...
	fmt.Println("  - mft=offset:    specifies the MFT shift from the partition beginning with an offset in the partition")
	fmt.Println("  - start=offset:  specifies the offset in the partition where the readind starts (partition beginning)")
	fmt.Println()
	...
	fmt.Println(" 4.", prog, "in=02_records.dat out=03_fslist.dat complete")
	fmt.Println(" 5.", prog, "in=03_fslist.dat out=04_dir.dat make-filelist")
	fmt.Println(" 6.", prog, "in=04_dir.dat out=recovered.file cp=12")
	fmt.Println()
